...
class BulletinPipeline:
    ...
    def run(self) -> Dict:
        ...
        try:
            ...
            try:
                raw_headlines = [item.get('title') for item in top_5_news]
                rewritten_headlines = self.viral_service.rewrite_headlines(raw_headlines)
                
                if len(rewritten_headlines) == 5:
                    for i in range(5):
                        top_5_news[i]['original_title'] = top_5_news[i]['title']
                        top_5_news[i]['title'] = rewritten_headlines[i]
                        logger.info(f"Rewrote: {top_5_news[i]['original_title'][:30]}... -> {rewritten_headlines[i]}")
                else:
                    logger.warning(f"Unexpected response length: {len(rewritten_headlines)}. Using originals.")
